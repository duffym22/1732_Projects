   ...
   class GSheetsAPI
   {
      ...
      private void Read_Attendance_Status()
      {
         ...
         try
         {
            ...
            //Treats the Google Sheet as the golden copy and
            ...
            for (int i = 1; i < idList.Count; i++)
            {
               ...
               IList<object> row = idList[i];

               //if we fail to parse the ID - don't add it to the list
               //numbers should be in a known and specific format
               ID = ulong.Parse((string)row[0]);
               name = row[1].ToString();
               stat = row[2].ToString();
      ...
      private void HandleException(Exception ex, string callingMethod)
      {
         ...
         _exMsg.AppendLine(string.IsNullOrEmpty(ex.InnerException.ToString()) ? "" : ex.InnerException.ToString());
